...
$.Autocompleter = function(input, options) {
	...
	$input.bind((window.opera ? "keypress" : "keydown") + ".autocomplete", function(event) {
		...
		current_q = jQuery(this).val();
		if(current_q.length==0 || current_q.length==1) {
			hideResultsNow();
			select.emptyList();
		}
	...
	function onChange(crap, skipPrevCheck) {
		...
		if ( currentValue.length >= options.minChars) {
			$input.next().removeClass("search_button").addClass("fa fa-cog fa-spin");
	...
	function autoFill(q, sValue, resultdata){
		// autofill in the complete box w/the first match as long as the user hasn't entered in more data
		// if the last user key pressed was backspace, don't autofill
		var whole_search_string = $input.val();
		if(whole_search_string.indexOf('>') === -1) {
			resultautofill = Array();	
			if(resultdata.length>0) {
				for(var i=1; i<resultdata.length; i++) {
					if( typeof resultdata[i].data[2] !== 'undefined') {
						resultautofill.push(resultdata[i].data[2]);
					}
				}
			}

			var autofilltext = '';
			if(resultautofill.length>0) {
				var matcher = new RegExp("^"+q+"", "i");
				for(var j=0; j<resultautofill.length; j++) {
					if( resultautofill[j].match(matcher) ) {
						autofilltext = resultautofill[j];
					}	
				}
			}
	...
	function stopLoading() {
		$input.next().removeClass("fa fa-spin fa-cog").addClass("search_button");
	};
