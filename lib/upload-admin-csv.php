...
define('VENDOR_ADD_CSV', 'vendor_add_csv');

class vendor_add_csv_page {
 
	function vendor_add_csv_page() {
		add_filter('screen_layout_columns', array(&$this, 'on_screen_layout_columns'), 10, 2);
		add_action('admin_menu', array(&$this, 'on_admin_menu'));
                add_action('admin_post_save_jeweldb_vendor_add_csv', array(&$this, 'on_save_changes'));
		add_action('wp_ajax_admin_product_zip_csv_upload', array($this, 'admin_product_zip_csv_upload_ajax'));
                add_action('wp_ajax_upload_admin_csv_products_images', array($this, 'upload_admin_csv_products_images_ajax'));
		add_action('wp_ajax_load_admin_products_from_csv', array($this, 'load_admin_products_from_csv'));
		add_action('wp_ajax_run_admin_uploads_products', array($this, 'run_admin_uploads_products_ajax'));
	...
	function on_admin_menu() {
		$this->pagehook = add_submenu_page('edit.php?post_type=product',__('VENDOR CSV',''), __('Vendor CSV File',''), 'manage_options', VENDOR_ADD_CSV, array(&$this, 'on_show_page'),'',90 );
	...
	function on_show_page() {
		...
		<input type="hidden" name="action" value="save_jeweldb_vendor_add_csv" />
...
//                print_r($_POST['vendor_add_csv']); exit;
		if ( !current_user_can('manage_options') )
		wp_die( __('Cheatin&#8217; uh?') );	
		//cross check the given referer
		check_admin_referer('theme-settings-metaboxes');
		//process here your on $_POST validation and / or option saving
		update_option('global_vendor_add_csv',serialize($_POST['vendor_add_csv']));
		wp_redirect($_POST['_wp_http_referer']);	
	}
      
        function user_csv_uploads_settings(){          
         $global_global_vendor_add_csv = unserialize(get_option('global_vendor_add_csv')); 
         $selected_user = $global_global_vendor_add_csv['csv_import_user']; ?>
...
//                            print_r($blogusers); 
//                            echo '<pre>'; ?>
                            <select name="vendor_add_csv[csv_import_user]" id="vendor_add_csv[csv_import_user]">
			...
			<div class="container" style="">
				<!--<h2 class="page-header"><i class="icon ion-android-note"></i>Upload Products as CSV</h2>-->
					...
					<div class="form-group upload_csv_product_images_row" style="text-align:center; display: none;">
						...
						<button class="btn btn-primary upload_product_images_first" id="admin_upload_csv_products_images">Upload Product Images</button>
					</div>
					...
					<div class="form-group upload_products_by_csv" style="text-align:center; margin-top:20px;">
						...
						<button class="btn btn-success" id="load_admin_csv_products" data-chunk="1">Load CSV Products</button>
					</div>
					<div class="form-group run_csv_upload" style="text-align:center; margin-top:20px;">
						<button class="btn btn-danger" id="start_admin_uploading_csv_products">Run Upload Products</button>
					</div>
    ...
    <?php
    echo ob_get_clean();    }
public function admin_product_zip_csv_upload_ajax() {
            check_ajax_referer('product_csv_zip_allow', 'nonce');
            //$merchant_user = wp_get_current_user();
            $global_global_vendor_add_csv = unserialize(get_option('global_vendor_add_csv')); 
            $merchant_user_id = $global_global_vendor_add_csv['csv_import_user'];
...
//                print_r($response); exit;
    ...
    public function mm_set__merchant_upload_dir( $upload ) {
    	if(isset($_REQUEST)) {
            if($_REQUEST['action']=='admin_product_zip_csv_upload') {
//                echo "hello";
                $global_global_vendor_add_csv = unserialize(get_option('global_vendor_add_csv')); 
                $merchant_user_id = $global_global_vendor_add_csv['csv_import_user'];
//                $merchant_user_id = $merchant_user->ID;
...
//            print_r(unzip_file( $file, $extract_dir ));exit;
    ...
    public function load_admin_products_from_csv($chunk) {
...
//        echo "hello"; exit;
        ...
        if($product_csv_chunks_flag=='false') {
            foreach($db_csvContents_chunks as $key => $values) { 
                    ...
                    $chunks_buttons .= '<button class="btn btn-danger" id="start_admin_uploading_csv_products" data-chunk="'.$key_chunk.'">Run Upload Products ('.$remaining_chunk.''.$count_products_into_chunks.' out of '.$total_csv_products.')</button>';
    ...
    public function run_admin_uploads_products_ajax() {
    	...
    	if($chunk!="") {
    		...
    		$load_csv = $this->load_admin_products_from_csv($chunk);
...
$vendor_add_csv_page = new vendor_add_csv_page();
$GLOBALS['vendor_add_csv'] = unserialize(get_option('global_vendor_add_csv'));
